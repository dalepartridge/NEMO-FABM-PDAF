...
CONTAINS
   ...
   SUBROUTINE dom_vvl_init
         ...
         DO jj = 1,jpj
            DO ji = 1,jpi
              !    zcoef = (tmask(ji,jj,jk) - wmask(ji,jj,jk))   ! 0 everywhere tmask = wmask, ie everywhere expect at jk = mikt
               ...
               gdep3w_n(ji,jj,jk) = gdept_n(ji,jj,jk) - sshn(ji,jj)
      ...
      ehv_b(:,:) = 0.
      DO jk = 1, jpkm1
         ehu_b(:,:) = ehu_b(:,:) + e3u_b(:,:,jk) * umask(:,:,jk)
         ehv_b(:,:) = ehv_b(:,:) + e3v_b(:,:,jk) * vmask(:,:,jk)
      END DO
      ehur_b(:,:) = umask_i(:,:) / ( ehu_b(:,:) + 1. - umask_i(:,:) )
      ehvr_b(:,:) = vmask_i(:,:) / ( ehv_b(:,:) + 1. - vmask_i(:,:) )
   ...
   SUBROUTINE dom_vvl_sf_nxt( kt, kcall ) 
      ...
      ehu_a(:,:) = 0._wp                        ! Ocean depth at U-points
      ehv_a(:,:) = 0._wp                        ! Ocean depth at V-points
      DO jk = 1, jpkm1
         ehu_a(:,:) = ehu_a(:,:) + e3u_a(:,:,jk) * umask(:,:,jk)
         ehv_a(:,:) = ehv_a(:,:) + e3v_a(:,:,jk) * vmask(:,:,jk)
      END DO
      !                                        ! Inverse of the local depth
      ehur_a(:,:) = 1._wp / ( ehu_a(:,:) + 1._wp - umask_i(:,:) ) * umask_i(:,:)
      ehvr_a(:,:) = 1._wp / ( ehv_a(:,:) + 1._wp - vmask_i(:,:) ) * vmask_i(:,:)
   ...
   SUBROUTINE dom_vvl_rst( kt, cdrw )
      ...
      IF( TRIM(cdrw) == 'READ' ) THEN        ! Read/initialise 
         ...
         IF( ln_rstart ) THEN                   !* Read the restart file
            ...
            IF( MIN( id1, id2 ) > 0 ) THEN       ! all required arrays exist
               CALL iom_get( numror, jpdom_autoglo, 'fse3t_b', e3t_b(:,:,:) )
               CALL iom_get( numror, jpdom_autoglo, 'fse3t_n', e3t_n(:,:,:) )
      ...
      ELSEIF( TRIM(cdrw) == 'WRITE' ) THEN   ! Create restart file
         ...
         CALL iom_rstput( kt, nitrst, numrow, 'fse3t_b', e3t_b(:,:,:) )
         CALL iom_rstput( kt, nitrst, numrow, 'fse3t_n', e3t_n(:,:,:) )
         !                                           ! ----------------------- !
