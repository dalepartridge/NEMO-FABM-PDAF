         ...
         DO jk = 2, jpkm1                            !* Slopes at u and v points
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  ...
                  zbu = MIN(  zbu, -100._wp* ABS( zau ) , -7.e+3_wp/e3u_n(ji,jj,jk)* ABS( zau )  )
                  zbv = MIN(  zbv, -100._wp* ABS( zav ) , -7.e+3_wp/e3v_n(ji,jj,jk)* ABS( zav )  )
         ...
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  ...
                  zck = ( gdepw_n(ji,jj,jk) - gdepw_n(ji,jj,mikt(ji,jj) ) ) / MAX( hmlp(ji,jj), 10._wp )
...
!                  zck = gdepw_n(ji,jj,jk)    / MAX( hmlp(ji,jj), 10. )
...
!!gm end modif
      ...
      ELSEIF ( lk_vvl ) THEN 
         ...
         DO jj = 2, jpjm1 
            DO ji = 2, jpim1   ! vector opt. 
               uslp(ji,jj,1) = -1./e1u(ji,jj) * ( gdept_b(ji+1,jj,1) - gdept_b(ji ,jj ,1) )  * umask(ji,jj,1) 
               vslp(ji,jj,1) = -1./e2v(ji,jj) * ( gdept_b(ji,jj+1,1) - gdept_b(ji ,jj ,1) )  * vmask(ji,jj,1) 
               wslpi(ji,jj,1) = -1./e1t(ji,jj) * ( gdepw_b(ji+1,jj,1) - gdepw_b(ji-1,jj,1) ) * tmask(ji,jj,1) * 0.5 
               wslpj(ji,jj,1) = -1./e2t(ji,jj) * ( gdepw_b(ji,jj+1,1) - gdepw_b(ji,jj-1,1) ) * tmask(ji,jj,1) * 0.5 
            END DO 
         END DO 
         ...
         DO jk = 2, jpk 
            DO jj = 2, jpjm1 
               DO ji = 2, jpim1   ! vector opt. 
                  uslp(ji,jj,jk) = -1./e1u(ji,jj) * ( gdept_b(ji+1,jj,jk) - gdept_b(ji ,jj ,jk) ) * umask(ji,jj,jk) 
                  vslp(ji,jj,jk) = -1./e2v(ji,jj) * ( gdept_b(ji,jj+1,jk) - gdept_b(ji ,jj ,jk) ) * vmask(ji,jj,jk) 
                  wslpi(ji,jj,jk) = -1./e1t(ji,jj) * ( gdepw_b(ji+1,jj,jk) - gdepw_b(ji-1,jj,jk) ) &
                    &                              * wmask(ji,jj,jk) * 0.5 
                  wslpj(ji,jj,jk) = -1./e2t(ji,jj) * ( gdepw_b(ji,jj+1,jk) - gdepw_b(ji,jj-1,jk) ) &
                    &                              * wmask(ji,jj,jk) * 0.5 
               END DO 
            END DO 
         END DO 
               ...
               DO ji = 1, jpim1
                  ...
                  ELSE
                    ...
                    zti_g_raw = (  zdxrho(ji+ip,jj,jk-kp,1-ip) / zdzrho(ji+ip,jj,jk-kp,kp)      &
                       &      - ( gdept_n(ji+1,jj,jk-kp) - gdept_n(ji,jj,jk-kp) ) / e1u(ji,jj)  ) * umask(ji,jj,jk)
            ...
            DO jk = 1, jpkm1
               ...
               DO jj = 1, jpjm1
                  DO ji = 1, jpim1         ! vector opt.
                     ...
                     zti_coord = znot_thru_surface * ( gdept_n(ji+1,jj  ,jk) - gdept_n(ji,jj,jk) ) / e1u(ji,jj)
                     ztj_coord = znot_thru_surface * ( gdept_n(ji  ,jj+1,jk) - gdept_n(ji,jj,jk) ) / e2v(ji,jj)                  ! unmasked
                    ...
                    !
                     ...
                     zbti = e1t(ji+ip,jj) * e2t(ji+ip,jj) * e3w_n(ji+ip,jj,jk+kp)
                     zbtj = e1t(ji,jj+jp) * e2t(ji,jj+jp) * e3w_n(ji,jj+jp,jk+kp)
                     !
   ...
   SUBROUTINE ldf_slp_mxl( prd, pn2, p_gru, p_grv, p_dzr )
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1
            ...
            !                        !- bound the slopes: abs(zw.)<= 1/100 and zb..<0
            !                           kxz max= ah slope max =< e1 e3 /(pi**2 2 dt)
            zbu = MIN(  zbu , -100._wp* ABS( zau ) , -7.e+3_wp/e3u_n(ji,jj,iku)* ABS( zau )  )
            zbv = MIN(  zbv , -100._wp* ABS( zav ) , -7.e+3_wp/e3v_n(ji,jj,ikv)* ABS( zav )  )
            ...
            DO jk = 1, jpk
               DO jj = 2, jpjm1
                  DO ji = 2, jpim1   ! vector opt.
                     uslp (ji,jj,jk) = -1./e1u(ji,jj) * ( gdept_b(ji+1,jj,jk) - gdept_b(ji ,jj ,jk) ) * umask(ji,jj,jk)
                     vslp (ji,jj,jk) = -1./e2v(ji,jj) * ( gdept_b(ji,jj+1,jk) - gdept_b(ji ,jj ,jk) ) * vmask(ji,jj,jk)
                     wslpi(ji,jj,jk) = -1./e1t(ji,jj) * ( gdepw_b(ji+1,jj,jk) - gdepw_b(ji-1,jj,jk) ) * tmask(ji,jj,jk) * 0.5
                     wslpj(ji,jj,jk) = -1./e2t(ji,jj) * ( gdepw_b(ji,jj+1,jk) - gdepw_b(ji,jj-1,jk) ) * tmask(ji,jj,jk) * 0.5
                  END DO
               END DO
            END DO
