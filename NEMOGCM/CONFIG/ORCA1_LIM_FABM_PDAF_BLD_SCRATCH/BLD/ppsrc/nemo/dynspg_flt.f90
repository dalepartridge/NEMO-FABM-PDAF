...
CONTAINS
   ...
   SUBROUTINE dyn_spg_flt( kt, kindic )
      ...
      IF( lk_vvl ) THEN          ! variable volume  (surface pressure gradient already included in dyn_hpg)
         ...
         ELSE                          ! flux form : applied on thickness weighted velocity
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1
                  DO ji = 2, jpim1   ! vector opt.
                     ua(ji,jj,jk) = (        ub(ji,jj,jk) * e3u_b(ji,jj,jk)      &
                        &           + z2dt * ua(ji,jj,jk) * e3u_n(ji,jj,jk)  )   &
                        &         / e3u_a(ji,jj,jk) * umask(ji,jj,jk)
                     va(ji,jj,jk) = (        vb(ji,jj,jk) * e3v_b(ji,jj,jk)      &
                        &           + z2dt * va(ji,jj,jk) * e3v_n(ji,jj,jk)  )   &
                        &         / e3v_a(ji,jj,jk) * vmask(ji,jj,jk)
                 END DO
               END DO
            END DO
            !
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1   ! vector opt.
            spgu(ji,jj) = e3u_a(ji,jj,1) * ua(ji,jj,1)
            spgv(ji,jj) = e3v_a(ji,jj,1) * va(ji,jj,1)
         END DO
      END DO
      DO jk = 2, jpkm1                     ! vertical sum
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               spgu(ji,jj) = spgu(ji,jj) + e3u_a(ji,jj,jk) * ua(ji,jj,jk)
               spgv(ji,jj) = spgv(ji,jj) + e3v_a(ji,jj,jk) * va(ji,jj,jk)
            END DO
         END DO
      END DO
