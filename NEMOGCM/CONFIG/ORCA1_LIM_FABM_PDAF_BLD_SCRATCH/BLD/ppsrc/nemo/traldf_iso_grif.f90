  ...
  SUBROUTINE tra_ldf_iso_grif( kt, kit000, cdtype, pgu, pgv,              &
            ...
            DO jk = 1, jpkm1
               DO jj = 1, jpjm1
                  DO ji = 1, jpim1
                     ...
                     zbu   = 0.25_wp * e1u(ji,jj) * e2u(ji,jj) * e3u_n(ji,jj,jk)
                     zah   = rldf * ahtu(ji,jj) * r_fact_lap(ji,jj,jk)                                  ! fsaht(ji+ip,jj,jk)
                     ...
                     zslope2 = zslope_skew + ( gdept_n(ji+1,jj,jk) - gdept_n(ji ,jj ,jk) ) * ze1ur * umask(ji,jj,jk+kp)
            ...
            DO jk = 1, jpkm1
               DO jj = 1, jpjm1
                  DO ji=1,jpim1
                     ...
                     zbv   = 0.25_wp * e1v(ji,jj) * e2v(ji,jj) * e3v_n(ji,jj,jk)
                     zah   = rldf * ahtv(ji,jj)                                  ! fsaht(ji,jj+jp,jk)
                     zslope_skew = triadj_g(ji,jj+jp,jk,1-jp,kp)
                     ! Subtract s-coordinate slope at t-points to give slope rel to s surfaces
                     !    (do this by *adding* gradient of depth)
                     zslope2 = zslope_skew + ( gdept_n(ji,jj+1,jk) - gdept_n(ji,jj,jk) ) * ze2vr * vmask(ji,jj,jk+kp)
                     zslope2 = zslope2 * zslope2
      ...
      IF( iom_use("uoce_eiv") .OR. iom_use("voce_eiv") .OR. iom_use("woce_eiv") )  THEN
         ...
         IF( ln_traldf_gdia .AND. cdtype == 'TRA' ) THEN
            ...
            DO jk=1,jpkm1
               zw3d(:,:,jk) = (psix_eiv(:,:,jk+1) - psix_eiv(:,:,jk))/e3u_n(:,:,jk)  ! u_eiv = -dpsix/dz
            END DO
            zw3d(:,:,jpk) = 0._wp
            CALL iom_put( "uoce_eiv", zw3d )    ! i-eiv current

            DO jk=1,jpk-1
               zw3d(:,:,jk) = (psiy_eiv(:,:,jk+1) - psiy_eiv(:,:,jk))/e3v_n(:,:,jk)  ! v_eiv = -dpsiy/dz
            END DO
            zw3d(:,:,jpk) = 0._wp
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         ...
         DO jk = 1, jpkm1
                  ...
                  DO kp = 0, 1
                     DO jj = 1, jpjm1
                        DO ji = 1, jpim1
                           ...
                           zah = rldf * ahtu(ji,jj) * r_fact_lap(ji,jj,jk)   !*umask(ji,jj,jk+kp)         !fsaht(ji+ip,jj,jk)           ===>>  ????
            ...
            DO jj = 2 , jpjm1
               DO ji = 2, jpim1  ! vector opt.
                  zbtr = 1._wp / ( e1t(ji,jj) * e2t(ji,jj) * e3t_n(ji,jj,jk) )
