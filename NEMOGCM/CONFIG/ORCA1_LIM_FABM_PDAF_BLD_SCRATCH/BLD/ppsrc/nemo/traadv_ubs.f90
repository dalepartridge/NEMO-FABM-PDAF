   ...
   SUBROUTINE tra_adv_ubs ( kt, kit000, cdtype, p2dt, pun, pvn, pwn,      &
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         ...
         DO jk = 1, jpkm1                                 ! Horizontal slab
            ...
            DO jj = 1, jpjm1            ! First derivative (gradient)
               DO ji = 1, jpim1   ! vector opt.
                  zeeu = e2u(ji,jj) * e3u_n(ji,jj,jk) / e1u(ji,jj) * umask(ji,jj,jk)
                  zeev = e1v(ji,jj) * e3v_n(ji,jj,jk) / e2v(ji,jj) * vmask(ji,jj,jk)
            ...
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  pta(ji,jj,jk,jn) = pta(ji,jj,jk,jn)                        &
                     ...
                     &                + ztv(ji,jj,jk) - ztv(ji  ,jj-1,jk)  ) / ( e1e2t(ji,jj) * e3t_n(ji,jj,jk) )
               END DO
            END DO
            !                                             
         END DO                                           !   End of slab
         ...
         DO jk = 1, jpkm1
            ...
            DO jj = 2, jpjm1
               DO ji = 2, jpim1   ! vector opt.
                  zbtr = 1. / ( e1t(ji,jj) * e2t(ji,jj) * e3t_n(ji,jj,jk) )
         ...
         IF( l_trd )  THEN                        ! vertical advective trend diagnostics
            DO jk = 1, jpkm1                       ! (compute -w.dk[ptn]= -dk[w.ptn] + ptn.dk[w])
               DO jj = 2, jpjm1
                  DO ji = 2, jpim1   ! vector opt.
                     zbtr = 1.e0 / ( e1t(ji,jj) * e2t(ji,jj) * e3t_n(ji,jj,jk) )
      ...
      DO jk = 1, jpkm1
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               ! up & down beta terms
               zbt = e1t(ji,jj) * e2t(ji,jj) * e3t_n(ji,jj,jk) / z2dtt
