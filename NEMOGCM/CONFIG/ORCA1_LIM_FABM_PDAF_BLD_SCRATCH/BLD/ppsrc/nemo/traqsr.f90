...
!CDIR NOVERRCHK
                  ...
                  DO jj = 1, jpj
                     DO ji = 1, jpi
                        zc0 = rn_abs * EXP( - e3t_n(ji,jj,1) * xsi0r     )
                        zc1 = zcoef  * EXP( - e3t_n(ji,jj,1) * zekb(ji,jj) )
                        zc2 = zcoef  * EXP( - e3t_n(ji,jj,1) * zekg(ji,jj) )
                        zc3 = zcoef  * EXP( - e3t_n(ji,jj,1) * zekr(ji,jj) )
         ...
         IF( ln_qsr_2bd ) THEN                            !  2 band light penetration !
            ...
            IF( lk_vvl ) THEN                                  !* variable volume
               ...
               IF ( ln_qsr_ice ) THEN
                  DO jj = 1, jpj
                     DO ji = 1, jpi
                        zc0 = zz0 * EXP( -gdepw_n(ji,jj,1)*xsi0r ) + zz1 * EXP( -gdepw_n(ji,jj,1)*xsi1r )
                        zc1 = zz0 * EXP( -gdepw_n(ji,jj,2)*xsi0r ) + zz1 * EXP( -gdepw_n(ji,jj,2)*xsi1r )
