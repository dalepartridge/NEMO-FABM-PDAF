...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namdyn_ldf in configuration namelist', lwp )
   ...
   SUBROUTINE ldf_dyn_c3d( ld_print )
      ...
      IF( ln_dynldf_lap ) THEN
         ...
         CALL ldf_zpf( .TRUE. , 1000., 500., 0.25, gdept_n(:,:,:), ahm1 )   ! vertical profile
         CALL ldf_zpf( .TRUE. , 1000., 500., 0.25, gdept_n(:,:,:), ahm2 )   ! vertical profile
         DO jk = 1,jpk
            DO jj = 1, jpj
               DO ji = 1, jpi
      ...
      IF( ln_dynldf_bilap ) THEN
         ...
         zh = MAX( zh, gdept_n(1,1,1) )   ! at least the first reach ahm0
         IF( ln_zco ) THEN               ! z-coordinate, same profile everywhere
            IF(lwp) WRITE(numout,'(36x," ahm ", 7x)')
            DO jk = 1, jpk
               IF( gdept_n(1,1,jk) <= zh ) THEN
                  zcoef(jk) = 1.e0
               ELSE
                  zcoef(jk) = 1.e0 + ( zr - 1.e0 )   &
                     &               * (  1. - EXP( ( gdept_n(1,1,jk   ) - zh ) / zh )  )   &
                     &               / (  1. - EXP( ( gdept_n(1,1,jpkm1) - zh ) / zh )  )
               ENDIF
         ...
         ELSE                            ! partial steps or s-ccordinate
            ...
            DO jk = 2, jpkm1
               DO jj = 1, jpj
                  DO ji = 1, jpi
                     IF( gdept_n(ji,jj,jk) <= zh ) THEN
                        ahm3(ji,jj,jk) = ahm3(ji,jj,1)
                        ahm4(ji,jj,jk) = ahm4(ji,jj,1)
                     ELSE
                        zd = 1.e0 + ( zr - 1.e0 ) * (  1. - EXP( ( gdept_n(ji,jj,jk) - zh ) / zh )  ) * zc
