      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1   ! vector opt.
            ...
            zuap = -zcoef0 * ( rhd   (ji+1,jj,1) + rhd   (ji,jj,1) + 2._wp * znad )   &
               &           * ( gdep3w_n(ji+1,jj,1) - gdep3w_n(ji,jj,1) ) / e1u(ji,jj)
            zvap = -zcoef0 * ( rhd   (ji,jj+1,1) + rhd   (ji,jj,1) + 2._wp * znad )   &
               &           * ( gdep3w_n(ji,jj+1,1) - gdep3w_n(ji,jj,1) ) / e2v(ji,jj)
            ! add to the general momentum trend
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               zuap = -zcoef0 * ( rhd   (ji+1,jj  ,jk) + rhd   (ji,jj,jk) + 2._wp * znad )   &
                  &           * ( gdep3w_n(ji+1,jj  ,jk) - gdep3w_n(ji,jj,jk) ) / e1u(ji,jj)
               zvap = -zcoef0 * ( rhd   (ji  ,jj+1,jk) + rhd   (ji,jj,jk) + 2._wp * znad )   &
                  &           * ( gdep3w_n(ji  ,jj+1,jk) - gdep3w_n(ji,jj,jk) ) / e2v(ji,jj)
               ! add to the general momentum trend
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1   ! vector opt.
            ...
            DO jk = 2, jpkm1
               ...
               zuap = - zcoef0 * ( rhd   (ji+1,jj  ,jk) + rhd   (ji,jj,jk) + 2._wp * znad )                    &
                  &            * ( gdep3w_n(ji+1,jj  ,jk) - gdep3w_n(ji,jj,jk) ) / e1u(ji,jj) * umask(ji,jj,jk-1)
               ...
               ! corrective term, we mask this term for the first wet level beneath the ice shelf (contribution done in the upper bloc)
               zvap = - zcoef0 * ( rhd   (ji  ,jj+1,jk) + rhd   (ji,jj,jk) + 2._wp * znad )                     &
                  &            * ( gdep3w_n(ji  ,jj+1,jk) - gdep3w_n(ji,jj,jk) ) / e2v(ji,jj) * vmask(ji,jj,jk-1)
               ! add to the general momentum trend
         ...
         DO ji = 2, jpim1
            ...
            IF (iku .GT. 1) THEN
               ...
               zuap            = -zcoef0 * ( rhd   (ji+1,jj  ,iku) + rhd   (ji,jj,iku) + 2._wp * znad )   &
                     &                   * ( gdep3w_n(ji+1,jj  ,iku) - gdep3w_n(ji,jj,iku) ) / e1u(ji,jj)
            ...
            IF (ikv .GT. 1) THEN
               ...
               zvap            = -zcoef0 * ( rhd   (ji  ,jj+1,ikv) + rhd   (ji,jj,ikv) + 2._wp * znad )   &
                     &                   * ( gdep3w_n(ji  ,jj+1,ikv) - gdep3w_n(ji,jj,ikv) )   / e2v(ji,jj)
      ...
      DO jk = 2, jpkm1
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               dzz  (ji,jj,jk) = gdep3w_n(ji  ,jj  ,jk) - gdep3w_n(ji,jj,jk-1)
               drhox(ji,jj,jk) = rhd   (ji+1,jj  ,jk) - rhd   (ji,jj,jk  )
               dzx  (ji,jj,jk) = gdep3w_n(ji+1,jj  ,jk) - gdep3w_n(ji,jj,jk  )
               drhoy(ji,jj,jk) = rhd   (ji  ,jj+1,jk) - rhd   (ji,jj,jk  )
               dzy  (ji,jj,jk) = gdep3w_n(ji  ,jj+1,jk) - gdep3w_n(ji,jj,jk  )
            END DO
         END DO
      END DO
...
!          true if de3w is really defined as the sum of the e3w scale factors as, it seems to me, it should be
      ...
      DO jj = 2, jpjm1
         DO ji = 2, jpim1   ! vector opt.
            rho_k(ji,jj,1) = -grav * ( e3w_n(ji,jj,1) - gdep3w_n(ji,jj,1) )               &
               &                   * (  rhd(ji,jj,1)                                    &
               &                     + 0.5_wp * ( rhd(ji,jj,2) - rhd(ji,jj,1) )         &
               &                              * ( e3w_n(ji,jj,1) - gdep3w_n(ji,jj,1) )   &
               &                              / ( gdep3w_n(ji,jj,2) - gdep3w_n(ji,jj,1) )  )
         END DO
      END DO
         ...
         DO jj = 2, jpjm1
            DO ji = 2, jpim1   ! vector opt.
               ...
               rho_k(ji,jj,jk) = zcoef0 * ( rhd   (ji,jj,jk) + rhd   (ji,jj,jk-1) )                                   &
                  &                     * ( gdep3w_n(ji,jj,jk) - gdep3w_n(ji,jj,jk-1) )                                   &
                  &            - grav * z1_10 * (                                                                     &
                  &     ( drhow (ji,jj,jk) - drhow (ji,jj,jk-1) )                                                     &
                  &   * ( gdep3w_n(ji,jj,jk) - gdep3w_n(ji,jj,jk-1) - z1_12 * ( dzw  (ji,jj,jk) + dzw  (ji,jj,jk-1) ) )   &
               ...
               rho_i(ji,jj,jk) = zcoef0 * ( rhd   (ji+1,jj,jk) + rhd   (ji,jj,jk) )                                   &
                  &                     * ( gdep3w_n(ji+1,jj,jk) - gdep3w_n(ji,jj,jk) )                                   &
                  &            - grav* z1_10 * (                                                                      &
                  &     ( drhou (ji+1,jj,jk) - drhou (ji,jj,jk) )                                                     &
                  &   * ( gdep3w_n(ji+1,jj,jk) - gdep3w_n(ji,jj,jk) - z1_12 * ( dzu  (ji+1,jj,jk) + dzu  (ji,jj,jk) ) )   &
               ...
               rho_j(ji,jj,jk) = zcoef0 * ( rhd   (ji,jj+1,jk) + rhd   (ji,jj,jk) )                                   &
                  &                     * ( gdep3w_n(ji,jj+1,jk) - gdep3w_n(ji,jj,jk) )                                   &
                  &            - grav* z1_10 * (                                                                      &
                  &     ( drhov (ji,jj+1,jk) - drhov (ji,jj,jk) )                                                     &
                  &   * ( gdep3w_n(ji,jj+1,jk) - gdep3w_n(ji,jj,jk) - z1_12 * ( dzv  (ji,jj+1,jk) + dzv  (ji,jj,jk) ) )   &
      ...
      DO jj = 1, jpj
        DO ji = 1, jpi
          ...
          ELSE IF(jk < jpkm1) THEN
             DO jkk = jk+1, jpk
                zrhh(ji,jj,jkk) = interp1(gdep3w_n(ji,jj,jkk),   gdep3w_n(ji,jj,jkk-1), &
                                         gdep3w_n(ji,jj,jkk-2), rhd(ji,jj,jkk-1), rhd(ji,jj,jkk-2))
             END DO
          ENDIF
        END DO
      END DO
